...
// cas de réponse non-2xx. Les services (VehiculeService, RecetteService...)
// passent tous par ce client plutôt que d'appeler fetch() directement.
...
async function requete(methode, chemin, corps) {
  ...
  if (!reponse.ok) {
    ...
    const erreur = new Error(donnees?.erreur || `Erreur serveur (${reponse.status}).`);
